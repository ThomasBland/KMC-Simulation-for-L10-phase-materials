...
def OneMonteCarloStep(LatticeDict, LatticeConstant, AtomCoords, AtomTypes, NearestNeighbourDictionary, Temperature, CurrentTime, DoWeNeedARecalc, SwapPossibilities, SwapPosRates, OutputYN):
    
    #Find the possible routes and their rates
    if DoWeNeedARecalc == 5:
        Temperature = (CurrentTime/60)*0.1
    ...
    return LatticeDict, AtomTypes, CurrentTime, DoWeNeedARecalc, SwapPossibilities, SwapPosRates, Timestep, Temperature, OutputYN

def Excecutable(NoXUCs, NoYUCs, NoZUCs, LatticeConstant, FeFraction, Temperature):
    #Initialise the system
    AtomCoords, AtomTypes, LatticeDict, NearestNeighbourDictionary, FeCount, NiCount = InitialiseSystem(NoXUCs,NoYUCs,NoZUCs, LatticeConstant, FeFraction)
    ...
    SwapPossibilities = list()
    SwapPosRates = list()
    ZOrderByCompletedNeighbours = 1.0
    OutputYN = True

    while ZOrderByCompletedNeighbours > 0.01:
        LatticeDict, AtomTypes, CurrentTime, DoWeNeedARecalc, SwapPossibilities, SwapPosRates, Timestep, Temperature, OutputYN = OneMonteCarloStep(LatticeDict, LatticeConstant, AtomCoords, AtomTypes, NearestNeighbourDictionary, Temperature, CurrentTime, DoWeNeedARecalc, SwapPossibilities, SwapPosRates, OutputYN)
        LRO, OrderMagnitude, AtomsWithCompletedNeighbours = MeasureOrder(AtomCoords, AtomTypes, NearestNeighbourDictionary, LatticeDict)
        XOrder = LRO.x
        YOrder = LRO.y
        ZOrder = LRO.z
        XOrderByCompletedNeighbours = AtomsWithCompletedNeighbours.x
        YOrderByCompletedNeighbours = AtomsWithCompletedNeighbours.y
        ZOrderByCompletedNeighbours = AtomsWithCompletedNeighbours.z
        SystemTotalEnergy = TotalSystemEnergy(AtomCoords, AtomTypes, NearestNeighbourDictionary, LatticeDict)
        #print(str(SystemTotalEnergy) + "," + str(round(OrderMagnitude*100,2)) + "," + str(round(CurrentTime/60,15)) + "," + str(Timestep) + "," + str(XOrder) + "," + str(YOrder) + "," + str(ZOrder) + "," + str(XOrderByCompletedNeighbours) + "," + str(YOrderByCompletedNeighbours) + "," + str(ZOrderByCompletedNeighbours))
        if OutputYN == True:
            print(str(round(CurrentTime/60,15)) + "," + str(ZOrder) + "," + str(ZOrderByCompletedNeighbours) + "," + str(Temperature)+ "," + str(Timestep))
            OutputYN = False
        #Heated at 0.5K per minute from 0K at 0 mins
        TrialTemperature = (CurrentTime/60)*0.1

        if TrialTemperature >= (Temperature + 0.1):

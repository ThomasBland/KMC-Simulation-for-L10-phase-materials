...
def Excecutable(NoXUCs, NoYUCs, NoZUCs, LatticeConstant, FeFraction, Temperature):
    ...
    while ZOrderByCompletedNeighbours < 1.0:
        MCStepCounter += 1
        LatticeDict, AtomTypes, CurrentTime, DoWeNeedARecalc, SwapPossibilities, SwapPosRates, Timestep = OneMonteCarloStep(LatticeDict, LatticeConstant, AtomCoords, AtomTypes, NearestNeighbourDictionary, Temperature, CurrentTime, DoWeNeedARecalc, SwapPossibilities, SwapPosRates)
        ...
        if MCStepCounter == 25:
            ...
            #print(str(round(CurrentTime/60,15)) + "," + str(ZOrder) + "," + str(ZOrderByCompletedNeighbours))
            #time.sleep(1)
            #SaveImage(str(CurrentTime/60))
            #time.sleep(1)
            try:
                SaveSystem(AtomCoords, AtomTypes, NearestNeighbourDictionary)
            except:
                pass
            MCStepCounter = 0
